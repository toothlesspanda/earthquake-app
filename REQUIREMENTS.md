...
1. SWA (Secure Web App)
   ...
   • Authenticated access to:
      o List[X]: A paginated list of recent earthquakes.
      o Detail[X]: A detail page for a selected earthquake.
   • Each earthquake (X) should include:
      o Title (e.g., location or event name)
      o Image (e.g., generated or fetched from a random image API)
      o Description (e.g., magnitude, depth, time)
   • The Detail[X] page should:
      o Use the output of the CLI app to generate a 3D visualization of the earthquake image.
      o Display metadata and processed image.

2. API + DB
  • Ingest earthquake data from a public source (e.g., USGS Earthquake API).
  • Store data in PostgreSQL (e.g., PostGIS).
  • Provide endpoints for:
    o Listing earthquakes with filters (e.g., magnitude, date)
    o Fetching details by ID
